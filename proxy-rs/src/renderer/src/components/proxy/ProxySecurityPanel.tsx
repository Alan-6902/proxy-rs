...
export function ProxySecurityPanel({
  config,
  setConfig,
  isRunning,
  isEn
}: ProxySecurityPanelProps): React.ReactNode {
  ...
  const handleRegenerateCert = useCallback(async () => {
    if (
      !(await askConfirm({
        title: isEn ? 'Regenerate self-signed certificate?' : '重新生成自签证书？',
        description: isEn
          ? 'The current certificate is replaced. Every client that trusted it must install the new one.'
          : '当前证书会被替换，所有已信任该证书的客户端都需要重新安装新证书。',
        confirmText: isEn ? 'Regenerate' : '重新生成',
  ...
  const handleRestart = useCallback(async () => {
    if (
      !(await askConfirm({
        title: isEn ? 'Restart proxy server now?' : '立即重启反代服务器？',
        description: isEn
          ? 'Streaming responses in flight will be interrupted and clients must retry.'
          : '正在进行的流式响应会被中断，客户端需要重试。',
        confirmText: isEn ? 'Restart' : '重启',
        cancelText: isEn ? 'Cancel' : '取消',
        tone: 'warning'
      }))
    )
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base">
              {isEn ? 'Security & Observability (v1.8)' : '安全与可观测设置 (v1.8)'}
            </CardTitle>
            {needsRestart && (
      ...
      {expanded && (
        <CardContent className="space-y-4 text-sm">
            ...
            <div className="flex items-center justify-between p-3 rounded-md bg-amber-500/10 border border-amber-500/20">
              <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400">
                ...
                <span className="text-xs">
                  {isEn
                    ? 'Configuration change requires a restart to take effect.'
                    : '配置已更改，重启后生效。'}
                </span>
              </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {isEn ? 'Reject larger requests with HTTP 413' : '超过则返回 HTTP 413'}
              </p>
            </div>
            <div>
              <Label className="text-xs">
                {isEn ? 'Rate limit (req/min per Key)' : '限速（每 Key 每分钟）'}
              </Label>
              <Input
                type="number"
                min={0}
                ...
                onChange={(e) =>
                  updateConfig('rateLimitPerKeyPerMinute', parseInt(e.target.value) || 0)
                }
                placeholder={isEn ? '0 = unlimited' : '0 = 不限制'}
                className="h-9"
              />
              <p className="text-xs text-muted-foreground mt-1">
                {isEn ? 'Anonymous → by IP' : '匿名时按 IP 限速'}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <textarea
                ...
                placeholder={
                  isEn
                    ? 'One per line, supports CIDR (e.g. 10.0.0.0/8)'
                    : '每行一个，支持 CIDR (如 10.0.0.0/8)'
                }
                className="w-full h-20 px-3 py-2 text-xs rounded-md border border-input bg-background"
              />
              <p className="text-xs text-muted-foreground mt-1">
                {isEn ? 'Empty = no restriction' : '为空 = 不限制'}
              </p>
            </div>
            <div>
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                {isEn ? 'IPv4 / IPv6 / CIDR' : '支持 IPv4 / IPv6 / CIDR'}
              </p>
            </div>
          </div>
                ...
                <label className="flex items-center gap-2 mt-2 cursor-pointer">
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      updateConfig('allowExternalWithoutApiKey', checked)
                    }
                    disabled={isRunning}
                  />
                  <span className="text-xs text-red-600 dark:text-red-400">
                    {isEn
                      ? 'I understand the risk, allow without API Key (DANGEROUS)'
                      : '我了解风险，允许无 Key 启动（危险）'}
                  </span>
                </label>
              </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center justify-between p-3 rounded-md border bg-background/50">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {isEn
                    ? 'Route same client to same account (prompt cache + anti-risk)'
            ...
            <div className="flex items-center justify-between p-3 rounded-md border bg-background/50">
              <div>
                <Label className="text-sm">
                  {isEn ? 'Prometheus /metrics' : 'Prometheus /metrics'}
                </Label>
                <p className="text-xs text-muted-foreground mt-1">
                  {isEn ? 'Expose monitoring metrics endpoint' : '暴露监控指标端点'}
                </p>
              </div>
              <Switch
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center justify-between p-3 rounded-md border bg-background/50">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {isEn ? 'Track config changes & critical events' : '记录配置变更与关键事件'}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <Switch
            ...
            <div>
              <Label className="text-xs">
                {isEn ? 'Recent requests limit' : '最近请求日志条数'}
              </Label>
              <Input
                type="number"
                min={20}
                ...
                onChange={(e) =>
                  updateConfig('recentRequestsLimit', parseInt(e.target.value) || 100)
                }
                className="h-9"
              />
              <p className="text-xs text-muted-foreground mt-1">
                {isEn ? 'Default 100, max 10000' : '默认 100，上限 10000'}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="text-xs">
                {isEn ? 'Keep-alive timeout (sec)' : 'keep-alive 空闲超时（秒）'}
              </Label>
              <Input
                type="number"
                min={5}
                ...
                onChange={(e) =>
                  updateConfig('keepAliveTimeoutMs', (parseInt(e.target.value) || 65) * 1000)
                }
                className="h-9"
              />
            </div>
            <div>
              <Label className="text-xs">
                {isEn ? 'HTTP fallback port (when TLS enabled)' : 'HTTP 回退端口（启用 TLS 时）'}
              </Label>
              <Input
                type="number"
                min={0}
          ...
          <div className="border-t pt-3">
            <div className="flex items-center justify-between mb-2">
              <Label className="text-sm font-medium">
                {isEn ? 'Self-signed TLS Certificate' : '自签 TLS 证书'}
              </Label>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div>
                    ...
                    <p className="font-mono">
                      {certInfo.notAfter ? new Date(certInfo.notAfter).toLocaleString() : '-'}
                    </p>
                  </div>
                  <div className="col-span-2">
                    <span className="text-muted-foreground">
                      {isEn ? 'SHA-256 Fingerprint:' : 'SHA-256 指纹:'}
                    </span>
                    <p className="font-mono text-2xs break-all">{certInfo.fingerprint || '-'}</p>
                  </div>
                  <div className="col-span-2">
                    <span className="text-muted-foreground">
                      {isEn ? 'Subject Alt Names:' : '备用名称 (SAN):'}
                    </span>
                    <p className="font-mono text-2xs">{certInfo.altNames?.join(', ') || '-'}</p>
                  </div>
                </div>
          ...
          {showAudit && (
            <div className="border-t pt-3">
              <div className="flex items-center justify-between mb-2">
                <Label className="text-sm font-medium">
                  {isEn ? 'Audit Log (recent 200)' : '审计日志（最近 200 条）'}
                </Label>
                ...
                {auditEntries.length === 0 ? (
                  ...
                  auditEntries
                    ...
                    .map((entry, i) => (
                      <div
                        key={i}
                        className="text-2xs font-mono p-1.5 rounded bg-background/50 border"
                      >
          ...
          {config.enableMetrics && isRunning && (
            <div className="flex items-center gap-2 p-2 rounded-md bg-blue-500/10 border border-blue-500/20 text-xs">
              ...
              <span className="text-muted-foreground">
                {isEn ? 'Metrics available at:' : '指标端点:'}
              </span>
