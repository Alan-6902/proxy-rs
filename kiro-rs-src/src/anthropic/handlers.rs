...
async fn handle_stream_request(
    ...
    // 调用 Kiro API（支持多凭据故障转移）；同时拿到实际服务本次请求的凭据 id，
    // 用于流结束后把 token 消耗归因到这张凭据
    let (response, credential_id) = match provider.call_api_stream_tracked(request_body).await {
        Ok(resp) => resp,
    ...
    let stream = create_sse_stream(
        ...
        ctx,
        initial_events,
        Some((provider.clone(), credential_id)),
    );
...
/// 缓冲流版本的记账，口径同 [`record_stream_usage`]
...
/// 创建 SSE 事件流
///
/// `usage_sink` 为 `Some((provider, credential_id))` 时，流结束（正常结束或读流出错）
/// 会把本次的 token 消耗累加到该凭据上。客户端提前断开时流被 drop，不会记账——
/// 宁可少记也不要凭半截数据编造消耗。
fn create_sse_stream(
    ...
    let processing_stream = stream::unfold(
        (body_stream, ctx, EventStreamDecoder::new(), false, interval(Duration::from_secs(PING_INTERVAL_SECS)), usage_sink),
        |(mut body_stream, mut ctx, mut decoder, finished, mut ping_interval, usage_sink)| async move {
            if finished {
                return None;
            }
                ...
                chunk_result = body_stream.next() => {
                    match chunk_result {
                        ...
                        Some(Err(e)) => {
                            ...
                            // 读流中断也记账：已经产生的输入输出确实消耗了额度
                            record_stream_usage(&usage_sink, &ctx);
...
async fn handle_non_stream_request(
    ...
    // 调用 Kiro API（支持多凭据故障转移）；同时拿到服务本次请求的凭据 id 用于记账
    let (response, credential_id) = match provider.call_api_tracked(request_body).await {
        Ok(resp) => resp,
        Err(e) => return map_provider_error(e),
    };
    ...
    // 把消耗记到实际服务本次请求的凭据上（与下发给客户端的 usage 同一份数字）
...
async fn handle_stream_request_buffered(
    ...
    // 创建缓冲 SSE 流
    let stream = create_buffered_sse_stream(response, ctx, Some((provider.clone(), credential_id)));
...
/// 4. 一次性发送所有事件
/// `usage_sink` 语义同 [`create_sse_stream`]：流结束时把 token 消耗记到该凭据。
            ...
            loop {
                tokio::select! {
                    ...
                    _ = ping_interval.tick() => {
                        ...
                        let bytes: Vec<Result<Bytes, Infallible>> = vec![Ok(create_ping_sse())];
                        return Some((stream::iter(bytes), (body_stream, ctx, decoder, false, ping_interval, usage_sink)));
                    }
                    ...
                    chunk_result = body_stream.next() => {
                        match chunk_result {
                            ...
                            Some(Err(e)) => {
                                ...
                                // 读流中断也记账：已产生的输入输出确实消耗了额度
                                record_buffered_usage(&usage_sink, &ctx);
                                let bytes: Vec<Result<Bytes, Infallible>> = all_events
                                    .into_iter()
                                    .map(|e| Ok(Bytes::from(e.to_sse_string())))
                                    .collect();
                                return Some((stream::iter(bytes), (body_stream, ctx, decoder, true, ping_interval, usage_sink)));
                            }
                            None => {
