...
import { Select, askConfirm } from '@/components/ui'
...
export function ApiKeyManager() {
  ...
  const handleDeleteKey = async (id: string) => {
    if (
      !(await askConfirm({
        title: isEn ? 'Delete this API key?' : '确定删除此 API Key？',
        description: isEn
          ? 'Clients using this key will be rejected immediately. This cannot be undone.'
          : '使用该 Key 的客户端将立即被拒绝，此操作不可恢复。',
  ...
  const handleResetUsage = async (id: string) => {
    if (
      !(await askConfirm({
        title: isEn ? 'Reset usage statistics?' : '确定重置用量统计？',
        description: isEn
          ? 'Recorded request and token counters for this key return to zero.'
          : '该 Key 已记录的请求数与 Token 计数将归零。',
