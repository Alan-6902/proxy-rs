  ...
  return (
    <div ref={dropdownRef} className={`relative ${className || ''}`}>
      ...
      >
        ...
        <ChevronDown
          className={`w-3.5 h-3.5 absolute right-1.5 top-1/2 -translate-y-1/2 text-muted-foreground transition-transform ${isOpen ? 'rotate-180' : ''}`}
        />
      </button>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-5 py-4 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold">
              {isEn ? 'Proxy Detailed Logs' : '反代详细日志'}
            </h2>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button
              variant="ghost"
              size="icon"
              className="rounded-full hover:bg-red-500 hover:text-white transition-colors"
              onClick={() => onOpenChange(false)}
            >
              <X className="w-4 h-4" />
            </Button>
            ...
            options={[
              {
                ...
                label: isEn ? 'All' : '全部',
                icon: (
                  <span className="w-2.5 h-2.5 rounded-full bg-gradient-to-r from-destructive via-warning to-primary" />
                )
              },
              {
                value: 'ERROR',
                label: 'ERR',
                icon: <span className="w-2.5 h-2.5 rounded-full bg-destructive" />
              },
              {
                value: 'WARN',
                label: 'WARN',
                icon: <span className="w-2.5 h-2.5 rounded-full bg-warning" />
              },
              {
                value: 'INFO',
                label: 'INFO',
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-2">
                        <span className="text-muted-foreground whitespace-nowrap flex-shrink-0 tabular-nums text-2xs">
                          {formatTime(log.timestamp)}
                        </span>
                        <Badge
                          variant="outline"
                          className={`text-3xs px-1 py-0 flex-shrink-0 font-semibold ${getLevelColor(log.level)}`}
                        >
                          {log.level}
                        </Badge>
                        <span className="text-primary/80 flex-shrink-0 font-medium text-2xs">
                          {log.category}
                        </span>
                        <span className="flex-1 break-all text-foreground/90 text-xs">
                          {log.message}
                        </span>
                      ...
                      {isExpanded && hasData && (
                        <pre className="mt-1.5 ml-20 p-2 rounded-md bg-muted/50 border border-border text-primary overflow-x-auto text-2xs leading-4">
        ...
        {/* 底部状态栏 */}
        <div className="flex-shrink-0 flex items-center justify-between px-4 py-1.5 border-t border-border text-2xs text-muted-foreground">
          <span>
          ...
          <div className="flex items-center gap-1">
            <ArrowDown
              className={`h-3 w-3 ${isAtBottom ? 'text-success' : 'text-muted-foreground/40'}`}
            />
            <span>
