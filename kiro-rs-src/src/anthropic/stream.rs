...
impl StreamContext {
    ...
    ///
    /// 与下发给客户端的 `message_delta.usage` 同一份数字：input 优先取自上游
    /// `contextUsageEvent` 反推的真实值，缺失时回退到请求前的本地估算。
    /// 供调用方在流结束后把消耗归到具体凭据上。
    pub fn final_token_usage(&self) -> (u64, u64) {
...
impl BufferedStreamContext {
    ...
    /// 本次请求最终计入的 `(input_tokens, output_tokens)`，口径同
    /// [`StreamContext::final_token_usage`]。
    pub fn final_token_usage(&self) -> (u64, u64) {
        let input = self
            .inner
            .context_input_tokens
