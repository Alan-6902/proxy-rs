...
import { DEFAULT_PROXY_POOL_CONFIG, type ProxyValidationResult } from '../../shared/proxyPool'
 ...
 *
 * 导出供主进程定时验活调度器（proxyPoolScheduler）复用，保证手动与定时验活走同一实现。
 */
export async function validateProxyEntry(params: {
  url: string
  testUrl?: string
  timeoutMs?: number
