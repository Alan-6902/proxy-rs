...
describe('Legacy Kiro RS main adapters', () => {
  ...
  it('serializes writers and blocks only unsafe startup recovery outcomes', async () => {
    ...
    expect(shouldBlockLegacyKiroRsMigrationAutoStart('rollback_available')).toBe(true)
    expect(shouldBlockLegacyKiroRsMigrationAutoStart('rollback_sync_required')).toBe(true)
    expect(shouldBlockLegacyKiroRsMigrationAutoStart('recovered')).toBe(false)
    expect(shouldBlockLegacyKiroRsMigrationAutoStart('none')).toBe(false)
  })
})
