...
export function stripAdminApiKey<TConfig extends { adminApiKey?: unknown }>(
  config: TConfig
): Omit<TConfig, 'adminApiKey'> {
...
export function assertDistinctAdminApiKey(
  config: Pick<ProxyConfig, 'adminApiKey' | 'apiKey' | 'apiKeys'>
): void {
  const adminApiKey = config.adminApiKey
  if (!adminApiKey) return
  if (
    adminApiKey === config.apiKey ||
    config.apiKeys?.some((apiKey) => apiKey.key === adminApiKey)
  ) {
...
export function switchAdminApiKey<TConfig extends { adminApiKey?: string }>(
  ...
  try {
    ...
    try {
      persistence.write(previousConfig)
    } catch {
      /* 只能返回泛化错误 */
    }
    try {
      runtime.update(previousConfig.adminApiKey)
    } catch {
      /* 只能返回泛化错误 */
    }
    return { success: false, error: ADMIN_KEY_UPDATE_ERROR }
