...
pub fn map_model(model: &str) -> Option<String> {
    ...
    // GPT-5.6（Kiro 2026-07-14 起支持）：sol / terra / luna
    // 裸 gpt-5.6 对齐 OpenAI 别名习惯 → sol
...
/// GPT-5.6（sol/terra/luna）在 Kiro 侧为 272K（非 OpenAI API 的 1.05M）
...
mod tests {
    ...
    fn test_map_model_unsupported() {
        ...
        // 仅 5.6 系列在 Kiro 侧可用，其它 GPT 版本不映射
    ...
    fn test_map_model_gpt_5_6() {
        ...
        // 连字符写法
        assert_eq!(map_model("gpt-5-6-terra"), Some("gpt-5.6-terra".to_string()));
        // 裸别名 → sol（对齐 OpenAI gpt-5.6 → sol）
        assert_eq!(map_model("gpt-5.6"), Some("gpt-5.6-sol".to_string()));
    }

    #[test]
    fn test_context_window_gpt_5_6() {
