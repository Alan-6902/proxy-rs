      ...
      <Card className="relative w-[900px] max-h-[80vh] shadow-2xl border-0 overflow-hidden animate-in fade-in zoom-in-95 duration-200 glass-card-strong">
        <CardHeader className="pb-3 border-b sticky top-0 z-10">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-sm">
            <span>
              {isEn ? 'Total' : '总计'}: <Badge variant="secondary">{logs.length}</Badge>
            </span>
            <span>
              {isEn ? 'Success' : '成功'}:{' '}
              <Badge className="bg-success/20 text-success">{successCount}</Badge>
            </span>
            <span>
              {isEn ? 'Error' : '错误'}:{' '}
              <Badge className="bg-destructive/20 text-destructive">{errorCount}</Badge>
            </span>
            <span className="text-muted-foreground">|</span>
            <span>
              Tokens {isEn ? 'Recent' : '最近'}:{' '}
              <Badge variant="outline">{recentTokens.toLocaleString()}</Badge>
            </span>
            ...
            <span>
              ...
              <Badge variant="outline">{recentCredits.toFixed(4)}</Badge>
            </span>
            <span className="flex items-center gap-1">
              Credits {isEn ? 'Total' : '总计'}:{' '}
              <Badge variant="secondary">{totalCredits.toFixed(4)}</Badge>
              {onResetCredits && (
                <Button
                  variant="ghost"
                  size="icon"
                ...
                <tbody className="font-mono">
                  {logs.map((log, idx) => (
                    <tr key={idx} className="border-b border-muted/30 hover:bg-muted/30">
                      ...
                      <td
                        className="p-2 truncate max-w-[150px] text-muted-foreground"
                        title={log.model}
                      >
                            ...
                            {expandedError === idx && (
                              <div className="absolute z-50 top-full left-1/2 -translate-x-1/2 mt-2 w-80 bg-background border border-destructive/30 rounded-lg shadow-xl p-3 text-left">
                                ...
                                <div className="text-xs font-sans">
                                  ...
                                  <pre className="whitespace-pre-wrap break-all bg-destructive/10 p-2 rounded text-destructive text-xs max-h-40 overflow-y-auto">
                                    {log.error}
                                  </pre>
                                </div>
                              </div>
