...
import { APP_ACCOUNT_EXPORT_TYPE, APP_PACKAGE_NAME } from '../../../../shared/appIdentity'
      ...
      <Card className="hover-lift">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="text-xs text-muted-foreground bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-lg p-3">
            {isEn
              ? 'Auto Refresh updates only this app’s account pool and account information before tokens expire.'
              : '自动刷新只更新本 App 内的账号池和账号信息，不会写入任何外部客户端凭据。'}
          </div>
...
function ConfigSyncCard({ isEn }: { isEn: boolean }): React.ReactNode {
  ...
  const handleExport = (): void => {
    ...
    a.href = url
    a.download = `${APP_PACKAGE_NAME}-config-${new Date().toISOString().replace(/[:.]/g, '-')}.json`
    a.click()
  ...
  const handleImport = async (): Promise<void> => {
    ...
    try {
      ...
      if (payload.type !== APP_ACCOUNT_EXPORT_TYPE) {
