export interface LegacyKiroRsMigrationCredentialRefreshLease {
  release(): void
}

const DEFAULT_CREDENTIAL_REFRESH_DRAIN_TIMEOUT_MS = 60_000

export class LegacyKiroRsMigrationCredentialRefreshGate {
  private admissionClosed = true
  private activeLeaseCount = 0
  private drainWaiters: Array<() => void> = []

  constructor(
    private readonly drainTimeoutMs = DEFAULT_CREDENTIAL_REFRESH_DRAIN_TIMEOUT_MS
  ) {}

  acquire(): LegacyKiroRsMigrationCredentialRefreshLease | null {
  ...
  async closeAdmissionAndDrain(): Promise<void> {
    this.closeAdmission()
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ...
        reject(new Error('Credential refresh drain timed out'))
      }, this.drainTimeoutMs)
