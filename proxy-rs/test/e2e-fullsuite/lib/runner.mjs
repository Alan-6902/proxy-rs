...
export async function runCases(cases, { base, token, only, reportDir }) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (error_) {
      ...
      if (error_.expected !== undefined)
        console.log(color('dim', `     expected: ${pretty(error_.expected, 200)}`))
      if (error_.actual !== undefined)
        console.log(color('dim', `     actual:   ${pretty(error_.actual, 200)}`))
      if (error_.hint) console.log(color('dim', `     hint:     ${pretty(error_.hint, 400)}`))
      if (status === 'error' && error_.stack)
        console.log(color('dim', `     stack:\n${indentStack(error_.stack)}`))
    }
